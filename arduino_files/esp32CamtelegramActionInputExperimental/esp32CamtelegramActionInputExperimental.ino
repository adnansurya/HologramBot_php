...
const char* serverName = "https://api.telegram.org/bot1564078066:AAHpbk7ryIumTCBIhMsxz4tvXF62KEEEDLk/getUpdates?offset=-1";
...
void loop()
{
  ...
  if (millis() - bot_lasttime > BOT_MTBS)
  {
    ...
    responseText =  httpGETRequest(serverName);
    updatePesan(responseText);
...
String lastUpdateId;
void updatePesan(String resText){
    int str_len = resText.length() + 1; 
    char res_array[str_len];
    resText.toCharArray(res_array, str_len);
  
    DynamicJsonDocument doc(1024);
    deserializeJson(doc, res_array);
    JsonObject obj = doc.as<JsonObject>();
    String hasil = obj["result"];
    String newUpdateId = obj["result"][0]["update_id"];
    
    
     if(newUpdateId != lastUpdateId){
        String pesan = obj["result"][0]["message"]["text"];
        String chatId = obj["result"][0]["message"]["chat"]["id"];
        String asalId = obj["result"][0]["message"]["from"]["id"];
        String asalFirstName = obj["result"][0]["message"]["from"]["first_name"];
        String asalLastName = obj["result"][0]["message"]["from"]["last_name"];
        String asalUsername = obj["result"][0]["message"]["from"]["username"];
        Serial.println("Pesan baru : " + pesan);
        if (pesan == "/capture" || pesan == "/capture@holocambot") {
          ledcAttachPin(4, 3);
          ledcSetup(3, 5000, 8);
          ledcWrite(3, 10);

          if(Start){
//            bot.sendMessage(chatId, "Mengambil gambar...");
//            Serial.println("ambil gambar");
            alerts2Telegram(token, chatId);
  
            if(chatId != hologram_id){
              bot.sendMessage(adnan_id, "Mengambil gambar dari " + asalFirstName + " " + asalLastName + " (@"+ asalUsername + ")");
            
              alerts2Telegram(token, adnan_id);
            }  
          }else{
              delay(1000);
          }
